...
/// Extract field names from a TraceEvent's schema (excludes the timestamp field).
fn field_names<T: TraceEvent>() -> Vec<String> {
    T::field_defs().iter().map(|f| f.name.clone()).collect()
}
...
fn kitchen_sink_all_field_types() {
    ...
    let names = field_names::<KitchenSink>();
    let decoded = KitchenSink::decode(ts, event, &names).unwrap();
...
fn empty_struct_round_trip() {
    ...
    let names = field_names::<Empty>();
    for (ts, values) in &events {
        let decoded = Empty::decode(*ts, values, &names).unwrap();
...
fn multi_event_stream_preserves_order() {
    ...
    let names = field_names::<SingleVarint>();
    for (i, (ts, values)) in events.iter().enumerate() {
        let decoded = SingleVarint::decode(*ts, values, &names).unwrap();
...
fn interleaved_event_types() {
    ...
    let sv_names = field_names::<SingleVarint>();
    let ts_names = field_names::<TwoStrings>();
    for i in 0..20 {
        let (tid, ts, vals) = &events[i * 2];
        assert_eq!(*tid, varint_tid);
        let sv = SingleVarint::decode(*ts, vals, &sv_names).unwrap();
        assert_eq!(sv.v, i as u64);

        let (tid, ts, vals) = &events[i * 2 + 1];
        assert_eq!(*tid, strings_tid);
        let decoded = TwoStrings::decode(*ts, vals, &ts_names).unwrap();
...
fn interleaved_string_pools() {
    ...
    let names = field_names::<MultiPool>();
    let e0 = MultiPool::decode(events[0].0, events[0].1, &names).unwrap();
    assert_eq!(e0.x, id_a);
    assert_eq!(e0.y, id_a);
    assert_eq!(e0.z, id_a);

    let e1 = MultiPool::decode(events[1].0, events[1].1, &names).unwrap();
    assert_eq!(e1.x, id_a);
    assert_eq!(e1.y, id_b);
    assert_eq!(e1.z, id_a);

    let e2 = MultiPool::decode(events[2].0, events[2].1, &names).unwrap();
    assert_eq!(e2.x, id_c);
    assert_eq!(e2.y, id_b);
    assert_eq!(e2.z, id_a);
...
fn string_pool_deduplication() {
    ...
    let names = field_names::<MultiPool>();
    let decoded = MultiPool::decode(ts, event, &names).unwrap();
...
fn stack_frames_edge_cases() {
    ...
    for (idx, addrs) in cases.iter().enumerate() {
        ...
        let names = field_names::<FrameEvent>();
        let decoded = FrameEvent::decode(ts, event, &names).unwrap();
...
fn f64_special_values_round_trip() {
    ...
    let names = field_names::<Floats>();
    let decoded = Floats::decode(ts, event, &names).unwrap();
...
fn f64_nan_round_trip() {
    ...
    let names = field_names::<NanEvent>();
    let decoded = NanEvent::decode(ts, event, &names).unwrap();
...
fn unicode_string_round_trip() {
    ...
    let names = field_names::<TwoStrings>();
    let decoded = TwoStrings::decode(ts, event, &names).unwrap();
...
fn large_bytes_field() {
    ...
    let names = field_names::<BlobEvent>();
    let decoded = BlobEvent::decode(ts, event, &names).unwrap();
...
fn varint_boundary_values() {
    ...
    let names = field_names::<Boundaries>();
    let d0 = Boundaries::decode(events[0].0, &events[0].1, &names).unwrap();
    assert_eq!((d0.a, d0.b, d0.c, d0.d), (127, 128, 16383, 16384));

    let d1 = Boundaries::decode(events[1].0, &events[1].1, &names).unwrap();
    assert_eq!(
        (d1.a, d1.b, d1.c, d1.d),
        (u8::MAX, u16::MAX, u32::MAX, u64::MAX)
    );

    let d2 = Boundaries::decode(events[2].0, &events[2].1, &names).unwrap();
...
fn decode_wrong_field_count_returns_none() {
    ...
    assert!(TwoStrings::decode(None, &vals, &ts_names).is_none());
    // Empty slice
    let sv_names = field_names::<SingleVarint>();
    assert!(SingleVarint::decode(None, &[], &sv_names).is_none());
}
...
fn string_map_round_trip() {
    ...
    let names = field_names::<MapEvent>();
    let d0 = MapEvent::decode(events[0].0, &events[0].1, &names).unwrap();
    ...
    let d1 = MapEvent::decode(events[1].0, &events[1].1, &names).unwrap();
    assert_eq!(d1.tags.count(), 0);

    let d2 = MapEvent::decode(events[2].0, &events[2].1, &names).unwrap();
...
fn many_events_many_types_stress() {
    ...
    let a_names = field_names::<TypeA>();
    let b_names = field_names::<TypeB>();
    let c_names = field_names::<TypeC>();
...
fn encoder_new_to_with_preallocated_buffer() {
    ...
    let names = field_names::<SingleVarint>();
    let decoded = SingleVarint::decode(events[0].0, &events[0].1, &names).unwrap();
...
fn for_each_event_with_derive_types() {
    ...
    dec.for_each_event(|ev| {
        let decoded = Ev::decode(ev.timestamp_ns, ev.fields, ev.field_names).unwrap();
        results.push((
...
fn for_each_event_resolves_interned_strings() {
    ...
    dec.for_each_event(|ev| {
        ...
        let decoded = TaskStart::decode(ev.timestamp_ns, ev.fields, ev.field_names).unwrap();
...
fn optional_field_some_round_trip() {
    ...
    dec.for_each_event(|ev| {
        ...
        let names = field_names::<WithOptionalString>();
        let decoded = WithOptionalString::decode(ev.timestamp_ns, ev.fields, &names).unwrap();
...
fn optional_u64_round_trip() {
    ...
    let mut events = Vec::new();
    dec.for_each_event(|ev| {
        let names = field_names::<WithOptionalU64>();
        let decoded = WithOptionalU64::decode(ev.timestamp_ns, ev.fields, &names).unwrap();
...
fn named_decode_missing_optional_field_returns_none() {
    ...
    let mut found = false;
    dec.for_each_event(|ev| {
        // Use WithOptionalString's field names to decode a WithoutOptional event.
        let names = field_names::<WithOptionalString>();
        let decoded = WithOptionalString::decode(ev.timestamp_ns, ev.fields, &names);
        // This should fail because the wire data has field names from WithoutOptional,
        // not WithOptionalString. The field_names from the wire are ["required_id"],
        // but we're passing WithOptionalString's names ["required_id", "optional_name"].
        // The named lookup finds "required_id" in ev.field_names, and "optional_name"
        // is missing from ev.field_names, so it calls decode_missing() -> Some(None).
        //
        // Actually, ev.field_names comes from the wire schema (WithoutOptional),
        // which only has ["required_id"]. The names we pass are the reader's expected
        // names. The decode zips ev.field_names with ev.fields, then looks up each
