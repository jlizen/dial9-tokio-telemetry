#!/usr/bin/env node
"use strict";
// Scale test for directory parsing: generates a synthetic multi-file dataset
// and validates the parallel/cache pipeline at scale.
//
// Usage:
//   node test_directory_scale.js              # default: 20 files
//   node test_directory_scale.js --large      # 200 files (slower, for CI or manual validation)
...
async function main() {
  ...
  if (!fs.existsSync(demoPath)) {
    console.error("demo-trace.bin not found");
    process.exit(1);
  }
  ...
  try {
    // ── Cold parse (populates cache) ──
    console.log("Cold parse (parallel + cache write):");
    const t0 = Date.now();
    const r1 = await parseTrace(dir);
    assert(r1.files.length === FILE_COUNT, `found ${FILE_COUNT} files`);

    let coldCount = 0;
    for await (const { trace } of r1) {
      coldCount++;
      if (coldCount === 1) {
    ...
    console.log(`  Cold parse: ${coldMs}ms (${(coldMs / FILE_COUNT).toFixed(0)}ms/file)\n`);

    // ── Warm parse (cache hit) ──
    console.log("Warm parse (cache hit):");
    const t1 = Date.now();
    const r2 = await parseTrace(dir);
    let warmCount = 0;
    for await (const { trace } of r2) {
      warmCount++;
      if (warmCount === 1) {
        assert(trace.events.length > 0, "cache hit: has events");
      }
    }
    ...
    console.log(`  Warm parse: ${warmMs}ms (${(warmMs / FILE_COUNT).toFixed(0)}ms/file)\n`);

    // ── Sampled parse ──
    console.log("Sampled parse:");
    const sampleN = Math.min(10, FILE_COUNT);
    const t2 = Date.now();
    const r3 = await parseTrace(dir, { sample: sampleN });
    assert(r3.files.length === sampleN, `sample: selected ${sampleN} files`);
    let sampleCount = 0;
    for await (const _ of r3) { sampleCount++; }
    const sampleMs = Date.now() - t2;
    assert(sampleCount === sampleN, `sample: iterated ${sampleN} files`);
    console.log(`  Sampled parse (${sampleN}): ${sampleMs}ms\n`);

    // ── Full pipeline on cached data ──
    console.log("Full pipeline on cached data:");
    const t3 = Date.now();
    const r4 = await parseTrace(dir);
    let worstPollMs = 0;
    let worstFile = "";
    let totalDelays = 0;
    let pipelineCount = 0;

    for await (const { file, trace } of r4) {
      ...
      for (const w of workerIds) {
        for (const p of spans.workerSpans[w].polls) {
          ...
          if (dur > worstPollMs) { worstPollMs = dur; worstFile = file; }
        }
      }
      pipelineCount++;
    }
    ...
    assert(worstPollMs > 0, `pipeline: found worst poll (${worstPollMs.toFixed(1)}ms in ${worstFile})`);
    assert(totalDelays > 0, `pipeline: found ${totalDelays} scheduling delays across all files`);
    console.log(`  Pipeline on cached data: ${pipelineMs}ms (${(pipelineMs / FILE_COUNT).toFixed(0)}ms/file)`);
    console.log(`  Worst poll: ${worstPollMs.toFixed(1)}ms in ${worstFile}`);
    console.log(`  Total scheduling delays: ${totalDelays}\n`);
